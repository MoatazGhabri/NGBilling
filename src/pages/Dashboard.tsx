...
export const Dashboard: React.FC = () => {
  ...
  const stats = {
    ...
    produitsEnStock: produits.reduce((sum, p) => sum + p.stock, 0),
      ...
      {stats.alertesStock > 0 && (
        <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-4 animate-pulse">
          ...
          <p className="text-sm text-yellow-700 dark:text-yellow-300 mt-1">
            {stats.alertesStock} produit(s) ont un stock faible. Consultez la section Produits pour plus de détails.
          </p>
        </div>
      )}
